// carte-subscription.js v6 — Carte paywall and subscription status
// ──────────────────────────────────────────────────────────────────
// Access rules:
...
window.Mise.carteSubscription = (function () {
  ...
  async function check(userId) {
    ...
    try {
      ...
      // Carte access: active with carte/suite/suite-all plan, or in trial
      var hasAccess = inTrial || (status === 'active' && (plan === 'carte' || plan === 'suite' || plan === 'suite-all'));
  ...
  function _updateSwitcher(plan, inTrial) {
    var veriqoBtn = document.getElementById('veriqo-switcher-btn');
    var yieldBtn = document.getElementById('yield-switcher-btn');
    if (veriqoBtn) {
      if (inTrial || plan === 'suite' || plan === 'suite-all') {
        veriqoBtn.style.cssText = 'display:flex;align-items:center;gap:6px;padding:7px 11px;background:rgba(255,255,255,0.11);border:1px solid rgba(255,255,255,0.18);border-radius:10px;cursor:pointer;flex-shrink:0';
        veriqoBtn.innerHTML = '<svg width="20" height="20" viewBox="0 0 512 512" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"><defs><linearGradient id="miseSwitchShield" x1="10%" y1="0%" x2="90%" y2="100%"><stop offset="0%" stop-color="#52D05C"/><stop offset="100%" stop-color="#1EA040"/></linearGradient></defs><rect width="512" height="512" rx="112" fill="#1B3A5C"/><path d="M250 82 Q118 112 118 112 L118 295 Q118 388 250 438 Q382 388 382 295 L382 112 Z" fill="url(#miseSwitchShield)"/><polyline points="163,295 228,368 366,212" stroke="white" stroke-width="46" stroke-linecap="round" stroke-linejoin="round" fill="none"/></svg>'
          + '<span style="font-size:13px;font-weight:600;color:#F5F0E8;font-family:inherit;letter-spacing:-0.2px">Veriqo</span>';
      } else if (plan === 'carte') {
        veriqoBtn.style.cssText = 'display:flex;align-items:center;gap:4px;padding:5px 10px;background:none;border:1px solid rgba(255,255,255,0.15);border-radius:10px;cursor:pointer;flex-shrink:0';
        veriqoBtn.innerHTML = '<span style="font-size:12px;color:rgba(245,240,232,0.4);font-family:inherit">Try Veriqo →</span>';
      } else {
        veriqoBtn.style.display = 'none';
      }
    }
    if (yieldBtn) {
      if (inTrial || plan === 'suite' || plan === 'suite-all' || plan === 'yield') {
        yieldBtn.style.cssText = 'display:flex;align-items:center;gap:6px;padding:7px 11px;background:rgba(255,255,255,0.11);border:1px solid rgba(255,255,255,0.18);border-radius:10px;cursor:pointer;flex-shrink:0';
        yieldBtn.innerHTML = '<svg width="20" height="20" viewBox="0 0 60 60" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"><rect width="60" height="60" rx="14" fill="#1C2B1E"/><path d="M15,45 L45,45 L30,15 Z" fill="#C8A96E"/></svg>'
          + '<span style="font-size:13px;font-weight:600;color:#F5F0E8;font-family:inherit;letter-spacing:-0.2px">Yield</span>';
      } else {
