// subscription.js v9 — Veriqo paywall and subscription status
// ─────────────────────────────────────────────────────────────
// Access rules:
...
window.Mise.subscription = (function () {
  ...
  async function check(userId) {
    ...
    try {
      ...
      // Veriqo access: active with veriqo/suite/suite-all/null plan, or in trial
      var hasAccess = inTrial || (status === 'active' && (plan === null || plan === 'veriqo' || plan === 'suite' || plan === 'suite-all'));
  ...
  function _updateSwitcher(plan, inTrial) {
    var carteBtn = document.getElementById('carte-switcher-btn');
    var yieldBtn = document.getElementById('yield-switcher-btn');
    if (carteBtn) {
      if (inTrial || plan === 'suite' || plan === 'suite-all') {
        carteBtn.style.cssText = 'display:flex;align-items:center;gap:6px;padding:7px 11px;background:#1C2B1E;border:1px solid #2E4030;border-radius:10px;cursor:pointer;flex-shrink:0';
        carteBtn.innerHTML = '<svg width="20" height="20" viewBox="0 0 60 60" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"><rect width="60" height="60" rx="14" fill="#1C2B1E"/><path d="M44.1,15.9 A20,20 0 1,0 44.1,44.1 L39.2,39.2 A13,13 0 1,0 39.2,20.8 Z" fill="#C8A96E"/></svg>'
        ...
        carteBtn.style.cssText = 'display:flex;align-items:center;gap:4px;padding:5px 10px;background:none;border:1px solid #e5e4de;border-radius:10px;cursor:pointer;flex-shrink:0';
        carteBtn.innerHTML = '<span style="font-size:12px;color:#aaa;font-family:inherit">Try Carte →</span>';
      } else {
    ...
    if (yieldBtn) {
      if (inTrial || plan === 'suite' || plan === 'suite-all' || plan === 'yield') {
        yieldBtn.style.cssText = 'display:flex;align-items:center;gap:6px;padding:7px 11px;background:#1C2B1E;border:1px solid #2E4030;border-radius:10px;cursor:pointer;flex-shrink:0';
        yieldBtn.innerHTML = '<svg width="20" height="20" viewBox="0 0 60 60" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"><rect width="60" height="60" rx="14" fill="#1C2B1E"/><path d="M15,45 L45,45 L30,15 Z" fill="#C8A96E"/></svg>'
          + '<span style="font-size:13px;font-weight:600;color:#C8A96E;font-family:inherit;letter-spacing:-0.2px">Yield</span>';
      } else {
