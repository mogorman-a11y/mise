...
### 2026-09-06 — dashboard.js v8 / dashboard.css v4: event-led redesign (commit `7abe110`, live on `main`)
Full rebuild of the Dashboard module. Private chefs don't work or log HACCP every day, so the dashboard is now **booking-led**, not a generic daily-HACCP dashboard.

- **Three render states**, chosen from data by `_buildContext()`:
  1. **Brand-new account** (no jobs/quotes/invoices/payments/costings/HACCP logs/dishes/menus) → a "Welcome to Veriqo" empty state, single "Create your first job" CTA, module cards read "Not set up yet" (no bare zeros).
  2. **No upcoming booking** → "No upcoming bookings" card + "Create a job" primary CTA, then `Needs attention` ∥ `Recent activity`, then module summaries.
  3. **Upcoming booking** (`getAllJobs()` has one with `eventDate >= today`) → a **Next booking card** (relative + absolute date/time, client, guests, arrival/service time, location, type, a derived status, one most-relevant next action) + a **preparation-progress checklist**, then `Needs attention`, then module summaries.
- **Derived booking status** (no status field exists): `Service day` (today) → `Confirmed` (accepted quote in `yield_quotes` OR `job.tabDepositPaid`) → `Quote sent` / `Quote in draft` → **`Planning`** (nothing else known — deliberately does *not* imply the client confirmed).
- **Preparation progress rows:** Menu, Client details (a proxy — real intake-form completion isn't stored anywhere the app reads), Allergen information (heuristic overlap check via `window.Veriqo.normalizeAllergen`, worded "Potential conflict", not a clinical determination), Prep & pack (`prepPackStatus(job.prepAndPack)`), Quote & payment (`yield_quotes`/`yield_invoices` by `job_id` + tab flags). **HACCP readiness** only appears when the service is today or tomorrow; it is a **counted, actionable task only on the service date** — the day before it is a non-actionable `info` row excluded from the readiness meter and from next-action selection. The applicable check list is derived from `haccp_settings.enabledTiles` and whether the job has a cooked menu, not a fixed five.
- **Needs attention:** draft quotes, stale sent quotes (≥5 days), overdue invoices (£ shown), other open invoices, other upcoming jobs missing covers/location/time/menu (the headline booking is excluded — the prep card covers it). Each opens the relevant **filtered** Costing/Menus screen. When Costing sync isn't confirmed fresh for the current uid, the all-clear copy softens ("…in your saved data. Open Costing to sync…") and `_refreshCosting()` fires one throttled `Mise.yieldSync.pull()` on open.
- **Contextual HACCP:** module card floats to the top with a "Service today/tomorrow" badge when near; otherwise last, showing "N days on record" with no "overdue" language.
- **Accessibility:** semantic `<button>`/`<a>` only (no clickable divs), `:focus-visible`, `aria-label`s, `role="progressbar"`, 44px targets, status by icon **and** text (never colour alone), collapsed "ready" rows use native `<details>`.
- **Desktop width:** `shell.css` caps every `.module-panel` at `calc(100% - sidebar)` while `.app` already reserves the sidebar via `padding-left`, so panels come out one sidebar-width too narrow. Scoped opt-out: `#module-dashboard.module-panel { max-width: none }` in `dashboard.css`. `.dbx` then centres at `max-width: 1120px` with a 2-up (`dbx-split`) / 3-up (`dbx-modules`) grid ≥900px. **Do not** remove that opt-out.
- **New globals:** `window.vqDash*` helpers (`vqDashGo/Haccp/Jobs/NewJob/ViewJob/PrepPack/Intake/Costing`) in `dashboard.js` — thin wrappers over `showModule`/`showScreen`/`filterInvoices`/`filterQuotes`/`showTab`/`startNewJob`/`calViewJob`/`openPrepAndPackForJob`/`openIntakeFormModal`, with a short `setTimeout` so deep-links wait for a lazily-mounted module. `vqDashCosting` bails if the target screen element isn't in the DOM (locked-plan tease).
- **`app.html`:** the old `.placeholder-panel` wrapper around `#dashboard-content` is gone (the module always overwrote it); `?v` bumps only.
- Reads only existing data — nothing invented. `renderPrepPackDashboardBanner()` (prep-pack.js) is no longer called by the dashboard but is left in place; the legacy `.db-*` CSS is retained for it.
- **Verification:** `node --test` (149/149 on `main`), a 7-state `vm`-sandboxed render harness against the real module, a handler/dependency wiring check, and a signed-in browser pass (desktop + mobile) on new-user and upcoming-booking states incl. the width fix. **Not** covered before ship: keyboard tab-through, the Costing deep-links / `_refreshCosting` against an account that actually has quotes/invoices. Additive change, one-commit revert if needed.
...
Fixed based on the **deployed** v19/v24 sources (the local repo copies had drifted stale — v19 already had the `yield_pay` client-payment branch, `STRIPE_WEBHOOK_SECRET_CONNECT`, and `starter_module` provisioning that the repo's stale v18 file didn't; those are preserved untouched):
- `stripe-webhook` v20: `checkout.session.completed` now retrieves the real subscription status (`mapStatus()`: passes through `active`/`trialing`/`past_due`/`unpaid`, normalises Stripe's `canceled` → this app's `cancelled`) instead of hardcoding `active`. Added `invoice.paid` as the actual positive signal that money arrived, guarded on `amount_paid > 0` so the £0 invoice Stripe issues at trial start doesn't flip a still-trialing user to `active`. `customer.subscription.updated`/`.deleted` and `invoice.payment_failed` untouched — already correct. Multi-secret signature verification loop untouched.
- `create-checkout` v2: carries over the profile's existing `trial_ends_at` as Stripe's `trial_end` (must be ≥48h in the future, Stripe's own minimum) instead of granting a new 14-day trial every time. No trial_ends_at, or one too close to now (e.g. a returning cancel-and-resubscribe customer, whose trial is always in the past) → no `trial_end` at all, card charged immediately at checkout. Deliberate fail-open-to-charging, not fail-open-to-free-access.
- New table `stripe_events` (id = Stripe event id) doubles as an idempotency guard: inserted immediately after signature verification; a primary-key conflict (`23505`) means Stripe already sent this event, and the handler returns `duplicate`/200 without reprocessing.
- Deleted `api/stripe-webhook.js` (an old Vercel serverless duplicate of this same handler, with its own separate set of price-ID env vars) — confirmed 404 in production (not wired to any route, Stripe was not delivering to it), so nothing else needs updating on the Veriqo side. **If a Stripe Dashboard webhook endpoint pointing at `getveriqo.co.uk/api/stripe-webhook` still exists, it needs removing there too** — this deletion only removes the (already-dead) code path, not a live Stripe registration.

**Required deploy order, don't skip it:** the access-gate widening (`subscription.js` v11 `hasAccess()`, above) had to go out and be verified live *before* this webhook change — otherwise every trialing user would have been locked out the instant the webhook started reporting `trialing` truthfully instead of lying `active`.
...
Developed on branch `security/vq-009-headers`, preview checked, merged to `main` 2026-07-22. **`CSP_ENFORCE` is still `false`** — before ever flipping it to `true`, check the browser console on a real production session for report-only violations and adjust the allowlist in `middleware.js` accordingly.
...
Note (not fixed here, out of this pass's scope): the same duplicate-declaration pattern also exists for `openVeriqo()` in both `menus.js` and `costing.js` — an old `window.location.href = '/app'` redirect shadowed later in each file by the correct `showModule('haccp')` version. Currently harmless (the winning definition is correct in both files), but it's the same bug class as this fix and VQ-007 — worth cleaning up under VQ-012 (dead code).
...
Not yet fixed from the same audit (see `VERIQO_AUDIT_2026-07-22.md` for full list): VQ-010 remaining auth/email rate-limit gaps, VQ-011 leaked provider error messages, VQ-012 remaining dead code (`api/create-checkout.js` stub, duplicate `yield-sync.js`, duplicate `openVeriqo()` — see VQ-008 entry above), VQ-013 test-coverage gaps. VQ-003 through VQ-009 fixed 2026-07-22, see entries above.
