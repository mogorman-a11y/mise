...
window.Mise.auth = (function () {
  ...
  function _buildAuthHTML() {
    ...
    return '<div id="mise-auth" style="position:fixed;inset:0;background:'+bgColor+';z-index:9999;overflow-y:auto;-webkit-overflow-scrolling:touch">'
      ...
      +     '<div style="font-size:16px;color:#aaa;margin-top:4px">'+tagline+'</div>'
      +   '</div>'
      + '</div>'
      ...
      +     'style="width:100%;padding:12px;border:1px solid #e5e4de;border-radius:8px;font-size:15px;margin-bottom:10px;outline:none;font-family:inherit;background:#fff">'
      +   '<div id="auth-password-wrap" style="position:relative">'
  ...
  // ── _showAuthLoading ──────────────────────────────────────────────────────
  // Shows a lightweight "Signing you in…" overlay (same #mise-auth ID so
  // hideAuthScreen() removes it correctly). Used when a Supabase token exists
  // in localStorage — avoids flashing the full login form on hard refresh.
  function _showAuthLoading() {
    if (document.getElementById('mise-auth')) return;
    var cfg = window.MISE_AUTH_CONFIG || {};
    ...
    var logoHTML  = cfg.logoHTML   ||
      ...
      + '<defs><linearGradient id="lbg" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#1B3A5C"/><stop offset="100%" stop-color="#1B5C72"/></linearGradient>'
      + '<linearGradient id="lsg" x1="10%" y1="0%" x2="90%" y2="100%"><stop offset="0%" stop-color="#52D05C"/><stop offset="100%" stop-color="#1EA040"/></linearGradient></defs>'
      + '<rect width="512" height="512" rx="112" fill="url(#lbg)"/>'
      + '<path d="M278 82 Q146 112 146 112 L146 295 Q146 388 278 438 Q410 388 410 295 L410 112 Z" fill="#1B5C72"/>'
      + '<path d="M250 82 Q118 112 118 112 L118 295 Q118 388 250 438 Q382 388 382 295 L382 112 Z" fill="url(#lsg)"/>'
      + '<polyline points="163,295 228,368 366,212" stroke="white" stroke-width="46" stroke-linecap="round" stroke-linejoin="round" fill="none"/>'
      + '</svg>';
    document.body.insertAdjacentHTML('beforeend',
      '<div id="mise-auth" style="position:fixed;inset:0;background:' + bg + ';z-index:9999;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:20px">'
      + '<div style="display:flex;align-items:center;gap:14px">'
      + logoHTML
      + '<div style="font-size:26px;font-weight:700;color:' + nameColor + ';letter-spacing:-0.5px">' + (cfg.name || 'Veriqo') + '</div>'
      + '</div>'
      + '<div style="font-size:14px;color:#888">Signing you in…</div>'
      + '</div>'
    );
  }

  // ── init ───────────────────────────────────────────────────────────────────
  // Called once on page load. If a Supabase session token is in localStorage,
  // shows a loading overlay (not the login form) while verifying — prevents the
  // login screen from flashing on hard refresh for users who are signed in.
  async function init() {
    // Check localStorage for any Supabase session token (key: sb-{ref}-auth-token)
    var _hasToken = Object.keys(localStorage).some(function(k) {
      return k.startsWith('sb-') && k.endsWith('-auth-token') && localStorage.getItem(k);
    });

    // If a session token exists show a loading overlay — avoids the login form
    // flashing on hard refresh for users who are already signed in.
    // If no token, show login immediately (no point waiting).
    if (_hasToken) {
      _showAuthLoading();
    } else {
      showAuthScreen();
    ...
    // Links are generated server-side via api/magic-link.js (consolidated handler
    // for all three apps) using the admin API. verifyOtp() is called directly —
    // no Supabase redirect needed, no PKCE code-verifier required, works in any browser.
    ...
    function _noSession() {
      _confirm();
      if (!_signedIn) { hideAuthScreen(); showAuthScreen(); }
    }
    ...
    supabaseClient.auth.onAuthStateChange(async (event, session) => {
      ...
      if (event === 'SIGNED_OUT') {
        if (_signedIn) { _signedIn = false; showAuthScreen(); }
        return;
      }
      ...
      // Do NOT call _noSession() here on null session — INITIAL_SESSION fires null during
      // the token-refresh cycle, before TOKEN_REFRESHED fires with the new session.
      // Let getSession() be the definitive source for "no session".
    });

    try {
      // Race getSession() against a 5 s timeout.
      // Fresh token: resolves in <100 ms. Expired token refresh: 1–4 s on normal networks.
      // If it hangs past 5 s the refresh has stalled — show login so the user isn't stuck.
      // onAuthStateChange will still fire TOKEN_REFRESHED if the refresh eventually
      // succeeds, which auto-signs the user back in without them typing credentials.
      var _gsRace = await Promise.race([
        supabaseClient.auth.getSession(),
        new Promise(function(resolve) {
          setTimeout(function() { resolve({ data: { session: null }, _timedOut: true }); }, 5000);
        })
      ]);
      var _gsSession = (_gsRace.data || {}).session || null;
      if (_gsSession && !_signedIn && !_showingResetForm) {
        _confirm();
        _signedIn = true;
        if (_isEmailConfirm) await _showEmailConfirmed();
        await onSignedIn(_gsSession.user);
      } else if (!_signedIn) {
        _noSession();
      }
    } catch (e) {
  ...
  function _tab(tab) {
    ...
    var pwWrap   = document.getElementById('auth-password-wrap');
  ...
  async function _sendMagicLink() {
    ...
    var app = (window.MISE_AUTH_CONFIG && window.MISE_AUTH_CONFIG.name === 'Yield') ? 'yield'
            : (window.MISE_AUTH_CONFIG && window.MISE_AUTH_CONFIG.name === 'Carte') ? 'carte'
            : 'veriqo';
    try {
      var res = await fetch('/api/magic-link', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ email: email, app: app, redirectTo: redirectTo }),
      });
      var json = await res.json();
      if (!res.ok) throw new Error(json.error || 'Failed to send');
      var appName = (window.MISE_AUTH_CONFIG && window.MISE_AUTH_CONFIG.name) || 'Veriqo';
      _setMsg('Check your inbox — we\'ve sent you a ' + appName + ' sign-in link.', 'ok');
  ...
  async function _forgot() {
    ...
    try {
      var res = await fetch('/api/magic-link', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ email: email, type: 'recovery', app: app, redirectTo: window.location.origin + window.location.pathname }),
      });
      var json = await res.json();
      if (!res.ok) throw new Error(json.error || 'Failed to send');
  ...
  async function signup(email, password, businessName, chefName) {
    ...
    if (result.data.user && result.data.session) {
      ...
      // Fire-and-forget welcome email — don't block sign-in on delivery
      var _signupApp = (window.MISE_AUTH_CONFIG && window.MISE_AUTH_CONFIG.name || 'Veriqo').toLowerCase();
      fetch('/api/welcome-email', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: result.data.user.email, name: chefName || businessName || null, app: _signupApp })
      }).catch(function() {});
      await onSignedIn(result.data.user);
    } else {
