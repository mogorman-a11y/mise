...
function buildJobsPDF(jobs){
  // See buildAllergenMatrixPDF's comment — same fix, applied proactively since
  // this uses the identical Blob+anchor-click delivery mechanism that was
  // confirmed broken on mobile Chrome (2026-07-21).
  var win = window.open('', '_blank');
  if (!win) { toast('Please allow pop-ups to view the booking report', 'err'); return; }
...
function buildAllergenMatrixPDF(job, dishes){
  // Opened via window.open()+document.write() rather than the Blob+anchor-click
  // trick used elsewhere in this file (see buildJobsPDF) — on mobile Chrome a
  // programmatic click on an <a target="_blank"> blob: URL can be routed
  // through the download/file-preview flow instead of a normal tab, which
  // ignores the page's own CSS/viewport entirely (reported 2026-07-21: content
  // rendered as a small fixed block instead of filling the screen). Opening the
  // window synchronously first, before any other work, keeps it within the
  // popup blocker's "user gesture" allowance.
  var win = window.open('', '_blank');
  if (!win) { toast('Please allow pop-ups to view the allergen matrix', 'err'); return; }
  ...
  var headerCells = ALLERGENS_14.map(function(a){
    return '<th title="'+esc(a)+'" style="padding:4px 1px;width:28px;min-width:28px;font-size:8px;font-weight:700;color:#1C2B1E;border:1px solid #D4C9B5;background:#EAF4EC;text-align:center;vertical-align:bottom;line-height:1.2">'+(ALLERGEN_SHORT_LABELS[a]||esc(a))+'</th>';
  }).join('');
  ...
  var rows = dishes.map(function(d){
    ...
    var cells = ALLERGENS_14.map(function(a){
      ...
      return '<td style="padding:4px 1px;text-align:center;border:1px solid #EFE9DE;font-size:12px;'+(has?'background:#FDECEC;color:#A32D2D;font-weight:700':'color:#D8D2C8')+'">'+(has?'✓':'—')+'</td>';
    }).join('');
    // No white-space:nowrap on the dish name — letting it wrap keeps this
    // column's width from being driven by the longest dish name, which was
    // the main reason the table needed as much horizontal scroll as it did.
    return '<tr><td style="padding:6px 8px;width:90px;max-width:90px;border:1px solid #D4C9B5;font-size:12px;font-weight:600;color:#1C2B1E">'+esc(d.dish)
      +(d.category?'<div style="font-size:9px;color:#A09890;font-weight:400">'+esc(d.category)+'</div>':'')
      +'</td>'+cells+'</tr>';
  }).join('');
  ...
  var html = '<!DOCTYPE html><html><head><meta charset="utf-8">'
    ...
    +'.landscape-hint{display:none;font-size:12px;color:#2D7A3A;background:#EAF4EC;border:1px solid #C7E2CB;border-radius:6px;padding:8px 12px;margin-bottom:14px}'
    +'@media (max-width:700px) and (orientation:portrait){.landscape-hint{display:block}}'
    +'@media print{@page{size:A4 landscape;margin:14mm}.no-print{display:none!important}body{padding:0}.doc{max-width:none;margin:0}.table-wrap{overflow-x:visible}}'
    +'</style></head><body>'
    +'<div class="toolbar no-print">'
    +'<button class="btn-pdf" onclick="window.print()">Save as PDF</button>'
    +'<button class="btn-close" onclick="window.close()">Close</button>'
    ...
    +  '</div>'
    +'</div>'
    +'<div class="landscape-hint no-print">↻ Rotate to landscape for the best view — this table is wide, and prints on A4 landscape.</div>'
    +'<div class="table-wrap"><table><thead><tr><th style="padding:6px 8px;width:90px;border:1px solid #D4C9B5;background:#EAF4EC;font-size:11px;text-align:left">Dish</th>'+headerCells+'</tr></thead>'
