...
// ═══════════════════════════════════════════ ALLERGEN LOG SCAN LABEL ═════════
async function handleAllergenScanLabel(event) {
  var file = event.target.files && event.target.files[0];
  if (!file) return;
  event.target.value = '';
  ...
  var btn = document.getElementById('al-scan-btn');
  var origHTML = btn ? btn.innerHTML : '';
  if (btn) { btn.innerHTML = '⏳ Scanning…'; btn.disabled = true; }
  ...
  try {
    ...
    // Use scoped query to avoid duplicate-ID collision with HACCP allergen form
    var dishEl = document.querySelector('#module-menus #al-dish');
    if (dishEl && data.ingredientName) dishEl.value = data.ingredientName;

    ALLERGENS_14.forEach(function(a) {
      var el = document.getElementById('al-a-' + a.replace(/\s/g, '_'));
      if (el) el.checked = (data.allergens || []).some(function(raw) {
        return raw.toLowerCase().replace(/\s+/g,'') === a.toLowerCase().replace(/\s+/g,'')
          || (a === 'Cereals containing gluten' && /cereal|gluten/i.test(raw))
          || (a === 'Sulphur dioxide' && /sulph|sulfite/i.test(raw));
      });
    });
    ...
    resetBtn(true);
    toast('Scan failed — check your connection and try again', 'err');
    console.error('[Allergen scan] fetch:', err);
  }
}
